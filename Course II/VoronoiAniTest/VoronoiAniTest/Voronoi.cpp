...
void Voronoi::build_line(VParabola * par, set<VPoint *>& line)
{
	if (!(par->isLeaf)) {
		VParabola * pl = par->Left();
		VParabola * pr = par->Right();
		build_line(pl, line);
		build_line(pr, line);
	}
	else {
		if (line.find(par->site) == line.end())
		line.insert(par->site);
	}
	return;
}
...
void	Voronoi::RemoveParabola(VEvent * e)
{
	...
	if (p0->site == p2->site) std::cout << "error - right and left parabola have the same focus!\n";
